              ...
              installingWorker.onstatechange = (): void => {
                  if (installingWorker.state === 'installed') {
                      if (navigator.serviceWorker.controller) {
                          ...
                          // but the previous service worker
                          // will still serve the older
...
function checkValidServiceWorker(swUrl: string, config?: Config): void {
  ...
  fetch(swUrl)
      ...
      (contentType != null && contentType.indexOf('javascript') === -1)
              ...
              // Probably a different app. Reload the page.
...
export function register(config?: Config): void {
  ...
  // The URL constructor is available in all browsers that support SW.
      ...
      if (publicUrl.origin !== window.location.origin) {
          // Our service worker won't work if PUBLIC_URL
          // is on a different origin
          // from what our page is served on.
          // This might happen if a CDN is used to
          // serve assets;
